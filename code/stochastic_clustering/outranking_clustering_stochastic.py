...
def perform_outranking(actions, limites, lam, beta, iter,p_dir, q_dir, p_inv, q_inv,iter_stochastic):
    ...
    #     for i in range(0, n_acc):
    #         for j in range(0, n_acc):
    #             if sigma_D_a[i][j] > lam: freq_sigma_D_a[i][j] = freq_sigma_D_a[i][j] + 1
    #             if sigma_D_a[j][i] > lam: freq_sigma_D_a[j][i] = freq_sigma_D_a[j][i] + 1
    #             if sigma_I_a[i][j] > lam: freq_sigma_I_a[i][j] = freq_sigma_I_a[i][j] + 1
    #             if sigma_I_a[j][i] > lam: freq_sigma_I_a[j][i] = freq_sigma_I_a[j][i] + 1
    # for i in range(0, n_acc):
    #     for j in range(0, n_acc):
    #         print(freq_sigma_D_a[i][j]/(2*iter_stochastic),
    #               freq_sigma_D_a[j][i]/(2*iter_stochastic),
    #               freq_sigma_I_a[i][j]/(2*iter_stochastic),
    #               freq_sigma_I_a[j][i]/(2*iter_stochastic))
    #     print(" ")
